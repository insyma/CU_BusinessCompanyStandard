...
<script runat="server">
    ...
    Sub Page_Load(ByVal Src As Object, ByVal E As EventArgs)
        ...
        If formularID <> "" Then
            ...
            mHBody = "<table border='1' cellspacing='1' cellpadding='1'>"                    
                ...
                For Each question As ContentUpdate.Question In CUContainer.Questions
                    ...
                    Else
                        ...
                        Else
                            ...
                            mHBody += "<td>" & Request.Form(formObjName) & "</td>"
                        End If
                    End If
                    mHBody += "</tr>"
                Next
    ...
    Sub Mail_Send()
        ...
        dim html as string = "<html><head><style></style></head><body>"
        dim bodystring as string = ""
        if not fo_intern.fields("emailfelderinemail").value = "" then
            bodystring = mHBody
        end if
        SendMail.Body = html & fo_intern.fields("textvorfelder_0").value &  bodystring & fo_intern.fields("textnachfelder_0").value & "</body></html>"
	...
	Sub Mail_Send_Confirmation()
        ...
        ' Subject
		If fo.Fields("con_betreff").value <> "" Then
            mSubject = fo.Fields("con_betreff").value
        Else
            If Request.Form("Subject") <> "" Then
                mSubject = Request.Form("Subject")
            Else
    ...
    Sub UserlistEntry()
        ...
        dim userlist as new contentupdate.objectset()
        userlist.load(Formular.containers("ConUserList").objectsets("conuserlist_culist").id)
        Dim newEntry As ContentUpdate.Container = userlist.AddEntry()
        newEntry.fields("datum").Value = now().toString("d.M.yyyy HH:mm")
        For Each formfield As String In Request.Form.Keys
           '' response.write("<br /> - " & formfield & " :: " & Request.Form(formfield))
            newEntry.Fields(formfield).Value = Request.Form(formfield)
        Next
