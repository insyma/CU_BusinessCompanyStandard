...
jQuery(function() {
    ...
    jQuery('#documentViewer').bind('onDocumentLoadedError',function(e,errMessage){
        // clear and create new structure before reload (only incase html5 not found, then do it with flash)
        if(viewerConfig.RenderingOrder == "html5,flash") {
            $('.flexpaper_viewer_container').remove();
            $('<div id="planBViewer" style="position:absolute;top:0; left:0; width:100%;height:100%;background-color:#222222"></div>').appendTo('body');

            viewerConfig.RenderingOrder = 'flash,html5';
            $('#planBViewer').FlexPaperViewer( { config: viewerConfig } );
        }
    });
