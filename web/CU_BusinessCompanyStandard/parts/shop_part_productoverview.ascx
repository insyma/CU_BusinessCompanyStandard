...
<script runat="server">
	...
	Sub Page_Load(ByVal Src As Object, ByVal E As EventArgs)
		if not container.fields("dezen_produktoverview_cat").value = "" then

			dim c as new ContentUpdate.container()
			c.load(cint(container.fields("dezen_produktoverview_cat").properties("value").value))
			c.load(c.parentid)
			tempPage.Load(c.pages("inc_shop_category_overview").id)
